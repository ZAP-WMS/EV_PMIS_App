...
import 'package:ev_pmis_app/PMIS/widgets/common_pdf_view.dart';
...
class ElectricalReportAdmin extends StatefulWidget {
  final String? userId;
  String? cityName;
  final String? depoName;
  int? selectedIndex;
  ElectricalReportAdmin(
      {super.key,
      this.userId,
  ...
  @override
  State<ElectricalReportAdmin> createState() => _ElectricalReportAdminState();
}
...
class _ElectricalReportAdminState extends State<ElectricalReportAdmin> {
  ...
  Future<void> getRowsForFutureBuilder() async {
    ...
    for (int i = 0; i < userIdList.length; i++) {
      QuerySnapshot userEntryDate = await FirebaseFirestore.instance
          ...
          .collection('userId')
          .doc(userIdList[i])
          .collection(tabForElec[widget.selectedIndex!])
          .get();
  ...
  Future<void> downloadPDF(String userId, String date, int decision) async {
    ...
    var fileName = 'Electrical_Checklist_$userId.pdf';
  ...
  Future<Uint8List> _generatePDF(
    ...
    DocumentSnapshot electricalFieldDocSnapshot = await FirebaseFirestore
        .instance
        .collection('ElectricalQualityChecklist')
        .doc('${widget.depoName}')
        .collection('userId')
    ...
    if (electricalFieldDocSnapshot.exists) {
      electricalMapData =
          electricalFieldDocSnapshot.data() as Map<String, dynamic>;
    }

    List<List<dynamic>> fieldData = [
      ['Project :', '${electricalMapData['projectName'] ?? ''}'],
      ['Date :', '${electricalMapData['date'] ?? ''}'],
      ['Location :', '${electricalMapData['location'] ?? ''}'],
      [
        'Component of structure : ',
        '${electricalMapData['componentName'] ?? ''}'
      ],
      ['Vendor / Sub Vendor :', '${electricalMapData['vendor'] ?? ''}'],
      ['Grid / Axis & Level :', '${electricalMapData['grid'] ?? ''}'],
      ['Drawing Number :', '${electricalMapData['drawing'] ?? ''}'],
      ['Type of Filling :', '${electricalMapData['filling'] ?? ''}'],
    ];
          ...
          return pw.Container(
              ...
              child: pw.Column(children: [
                pw.Row(
                    ...
                    children: [
                      pw.Text(
                          'Electrical Quality Report / ${completeTabForElectrical[widget.selectedIndex!]}Table',
                          textScaleFactor: 2,
